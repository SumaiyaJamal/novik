<div class="relative" id="user-poper">
    <div class="h-8 cursor-pointer w-8 rounded-full overflow-hidden flex items-center justify-center bg-gray-300" @if(auth()->user() && auth()->user()->name) style="background-color:black" @endif>
    ...
    <div id="poper-tab" class="hidden absolute top-10 rounded-lg shadow right-5 bg-white z-50 w-min  h-max px-8 py-4">
        ...
        <ul class="mt-5 flex flex-col gap-3 font-semibold text-gray">
            <a href="{{ route('myquestions') }}" class="flex gap-1 cursor-pointer hover:brightness-105">
